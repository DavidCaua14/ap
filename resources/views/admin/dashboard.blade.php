...
<body>
    <nav class="navbar navbar-expand-md navbar-light fixed-top py-4">
        <div class="container">
            ...
            <form class="d-flex" role="search" action="{{ route('admin.objeto.search') }}" method="GET">
                <input class="form-control me-2" type="search" name="search" placeholder="Pesquisar objeto" aria-label="Search" onkeydown="if(event.key === 'Enter'){this.form.submit();}">
            </form>            
        ...
        <main>
            ...
            <div class="container">
                ...
                @if(isset($message))
                    <p class="alert alert-info my-3 mb-0 text-center">{{ $message }}</p>
                @endif
        
                @if($objetos->isEmpty() && !isset($message))
            ...
            @forelse($objetos as $objeto)
                <table class="table table-striped containerTable">
                    ...
                    <tbody>
                        <tr>
                            <td style="min-width: 150px; word-break: break-word;">
                                {{ $objeto->descricao }}
                            </td>
                            <td>
                                @if($objeto->imagem)
